...
public class ProjectResource {
	...
	@DELETE
	@Path("/project/{projectVersionId}")
	@RolesAllowedIfWebVersion({ "User", "Admin" })
	public void deleteProjectVersion(@RestPath Long projectVersionId) {
		if (appMode.equals("web")) {
			Long userId = Long.parseLong(jwt.getClaim(USER_ID).toString());
			usecase.deleteProjectVersion(userId, projectVersionId);
			return;
		}
		usecase.deleteProjectVersion(projectVersionId);
	}
}
