...
public class ProcessModelUsecaseTest {
    ...
    public void testSaveProcessModel_NewProcessModel() {
        ...
        ProcessModel processModel = new ProcessModel(TEST_PROCESS_NAME, TEST_PROCESS_XML, EMPTY_PROCESS_EVENTS, EMPTY_CALL_ACTIVITIES, EMPTY_DATA_STORES, TEST_DESCRIPTION,
        ...
        verify(repository, times(1)).findByNameOrBpmnProcessId(TEST_PROCESS_NAME, TEST_BPMN_PROCESS_ID, TEST_PROJECT_ID);
    ...
    public void testSaveProcessModel_NewProcess_CreateIfExistingIsCollaboration() {
        ...
        when(repository.findByNameOrBpmnProcessId(TEST_PROCESS_NAME, TEST_BPMN_PROCESS_ID, TEST_PROJECT_ID))
                .thenReturn(existingCollaboration);
    ...
    @Test
    public void testSaveProcessModel_ChildModelWithoutDescription() {
        when(processOperations.getBpmnProcessId(TEST_PROCESS_XML)).thenReturn(TEST_BPMN_PROCESS_ID);
        when(repository.findByNameOrBpmnProcessId(TEST_PROCESS_NAME, TEST_BPMN_PROCESS_ID, TEST_PROJECT_ID))
                .thenReturn(null);
        ...
        ProcessModel processModel = new ProcessModel(TEST_PROCESS_NAME, TEST_PROCESS_XML, EMPTY_PROCESS_EVENTS, EMPTY_CALL_ACTIVITIES, EMPTY_DATA_STORES, "",
                TEST_BPMN_PROCESS_ID, PARENT_BPMN_PROCESS_ID, ProcessType.PARTICIPANT);
        //

        when(repository.saveProcessModel(eq(TEST_PROJECT_ID), eq(processModel))).thenReturn(TEST_PROCESS_MODEL_ID);

        Long result = processModelUsecase.saveProcessModel(TEST_PROJECT_ID, TEST_PROCESS_NAME, TEST_PROCESS_XML,
                "", "false", PARENT_BPMN_PROCESS_ID);
    ...
    public void testSaveProcessModel_Replace() {
        ...
        doNothing().when(processMapRepository).copyConnections(TEST_PROJECT_ID, TEST_OLD_PROCESS_ID, TEST_NEW_PROCESS_ID);
        doNothing().when(processMapRepository).copyMessageFlowsAndRelations(TEST_PROJECT_ID, TEST_OLD_PROCESS_ID, TEST_NEW_PROCESS_ID);
        doNothing().when(repository).deleteProcessModel(TEST_OLD_PROCESS_ID);

        Long processId = processModelUsecase.saveProcessModel(TEST_PROJECT_ID, TEST_PROCESS_NAME, TEST_PROCESS_XML, TEST_DESCRIPTION, "false", null);
        ...
        verify(processOperations, times(1)).getDataStores(TEST_PROCESS_XML);
        verify(repository, times(1)).saveProcessModel(eq(TEST_PROJECT_ID), eq(processModel));
        verify(processMapRepository, times(1)).copyConnections(TEST_PROJECT_ID, TEST_OLD_PROCESS_ID, TEST_NEW_PROCESS_ID);
        verify(processMapRepository, times(1)).copyMessageFlowsAndRelations(TEST_PROJECT_ID, TEST_OLD_PROCESS_ID, TEST_NEW_PROCESS_ID);
    ...
    public void testReplaceProcessModel_IsCollaboration() {
        ...
        Exception exception = assertThrows(IllegalArgumentException.class, () -> {
            processModelUsecase.replaceProcessModel(TEST_PROJECT_ID, TEST_OLD_PROCESS_ID,
                    REPLACED_PROCESS_NAME, TEST_PROCESS_XML, TEST_DESCRIPTION, null);
        });
    ...
    public void testGetProcessInformation() {
        ...
        when(repository.getProcessInformation(TEST_PROJECT_ID)).thenReturn(processInformationList);

        List<ProcessInformation> result = processModelUsecase.getProcessInformation(TEST_PROJECT_ID);

        assertEquals(processInformationList, result);
        verify(repository, times(1)).getProcessInformation(TEST_PROJECT_ID);
    }
    ...
    public void testGetProcessDetails() {
        ...
        when(repository.getProcessDetails(TEST_PROCESS_MODEL_ID)).thenReturn(processDetails);

        ProcessDetails result = processModelUsecase.getProcessDetails(TEST_PROCESS_MODEL_ID);

        assertEquals(processDetails, result);
        verify(repository, times(1)).getProcessDetails(TEST_PROCESS_MODEL_ID);
    }

    @Test
    public void testSaveProcessModel_Collaboration() {
        when(processOperations.getBpmnProcessId(TEST_PROCESS_XML)).thenReturn(TEST_BPMN_PROCESS_ID);
        when(repository.findByNameOrBpmnProcessId(TEST_PROCESS_NAME, TEST_BPMN_PROCESS_ID, TEST_PROJECT_ID)).thenReturn(null);
        ...
        ProcessModel collaborationModel = new ProcessModel(TEST_PROCESS_NAME, TEST_PROCESS_XML, EMPTY_PROCESS_EVENTS, EMPTY_CALL_ACTIVITIES, EMPTY_DATA_STORES, TEST_DESCRIPTION,
        ...
        when(repository.saveProcessModel(eq(TEST_PROJECT_ID), eq(collaborationModel))).thenReturn(TEST_PROCESS_MODEL_ID);

        // SAVE PARTICIPANT
        when(processOperations.getBpmnProcessId(PARTICIPANT_XML_1)).thenReturn(PARTICIPANT_BPMN_ID);
        when(repository.findByNameOrBpmnProcessId(PARTICIPANT_NAME_1, PARTICIPANT_BPMN_ID, TEST_PROJECT_ID)).thenReturn(null);
        ...
        ProcessModel participantModel = new ProcessModel(PARTICIPANT_NAME_1, PARTICIPANT_XML_1, EMPTY_PROCESS_EVENTS, EMPTY_CALL_ACTIVITIES, EMPTY_DATA_STORES, PARTICIPANT_DESCRIPTION_1,
        ...
        when(processOperations.getMessageFlows(eq(TEST_PROCESS_XML), eq(bpmnIdToIdMap))).thenReturn(messageFlowDetailsList);
        ...
        verify(repository, times(1)).findByNameOrBpmnProcessId(TEST_PROCESS_NAME, TEST_BPMN_PROCESS_ID, TEST_PROJECT_ID);
        verify(repository, times(1)).findByNameOrBpmnProcessId(PARTICIPANT_NAME_1, PARTICIPANT_BPMN_ID, TEST_PROJECT_ID);
