    ...
    <v-card-text class="pa-0 mb-3">
      ...
      <div class="d-flex align-center justify-space-between">
        <div class="d-flex align-center">
          ...
          <div class="d-flex flex-column">
            <span class="text-caption">{{ $t('authentication.password') }}</span>
            <span>************</span>
          </div>
        </div>
        <div class="d-flex align-center justify-end">
          <v-btn variant="text" color="primary" @click="resetMessageAndOpenDialog(SelectedDialog.CHANGE_PW)">
    ...
    <div v-if="projects.length > 0">
      ...
      <v-table density="compact">
        ...
        >
          <td>
            <v-tooltip location="left">
              <template v-slot:activator="{ props }">
                <v-btn variant="plain" color="primary" v-bind="props" class="text-none table-btn"
                       prepend-icon="mdi-folder" @click="openProject(project.id)">
                  {{ project.name }}
                </v-btn>
              </template>
          ...
          <td class="text-no-wrap">{{ project.version }}</td>
        </tr>
        </tbody>
      </v-table>
    </div>
