    ...
    <v-card class="pa-5">
      ...
      <v-card-text class="mb-3">
        ...
        <div class="d-flex align-center justify-space-between">
          <v-btn type="button" height="50" color="primary" @click="patchUser(userId)">
            {{ $t('manageUsers.saveChanges') }}
          </v-btn>
          <v-btn  v-if="ownUserId !== userId" type="button" height="50" variant="text"
                  color="red" @click="deleteUser(userId)">
            {{ 'Delete User' }}
          </v-btn>
        </div>
      </v-card-text>
...
import {emailRules, firstNameRules, lastNameRules, newPasswordRules} from "@/components/Authentication/formValidation";
...
export default defineComponent({
  ...
  methods: {
    ...
    async patchUser(id: number) {
      ...
      const data = {
        email: this.localUserEmail != this.userEmail ? this.localUserEmail : null,
        firstName: this.localUserFirstName != this.userFirstName ? this.localUserFirstName : null,
        lastName: this.localUserLastName != this.userLastName ? this.localUserLastName : null,
